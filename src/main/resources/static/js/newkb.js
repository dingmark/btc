   ...
   // locateposition('bian',event.data);
    ...
    switch (type) {
        case'BTC':
            ...
            putask1to('kb',bz,'BTC',temp.asks[0][0]);
            var max=findmax(bz);
            symbol=symbol.replace('-','');
            var result=cacule('kb',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
            {
                newedrawtable(old,bz,'kb',symbol,temp.bids[0][0],result.percent,result.sell_trade,result.sell_symbol,result.sellprice)
            }
            break;
        case 'SDT':
            ...
            kbvar[bz][symbol].asks= temp.asks;
            kbvar[bz][symbol].bids= temp.bids;
            putask1to('kb',bz,'USDT',temp.asks[0][0]);
            var max=findmax(bz);
            symbol=symbol.replace('-','');
            var result=cacule('kb',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
        ...
        case 'ETH':
            ...
            kbvar[bz][symbol].asks= temp.asks;
            kbvar[bz][symbol].bids= temp.bids;
            putask1to('kb',bz,'ETH',temp.asks[0][0]);
            symbol=symbol.replace('-','');
            var max=findmax(bz);
            var result=cacule('kb',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
