   ...
   // locateposition('bian',event.data);
    ...
    switch (type) {
        case'BTC':
            ...
            putask1to('mc',bz,'BTC',temp.bids[0][0]);
            var max=findmax(bz);
            symbol=symbol.replace('_','');
            var result=cacule('mc',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
            {
                newedrawtable(old,temp,bz,'mc',symbol,temp.bids[0][0],result.percent,result.sell_trade,result.sell_symbol,result.sellprice)
            }
            break;
        case 'SDT':
            ...
            mcvar[bz][symbol].asks= temp.asks;
            mcvar[bz][symbol].bids= temp.bids;
            putask1to('mc',bz,'USDT',temp.bids[0][0]);
            var max=findmax(bz);
            symbol=symbol.replace('_','');
            var result=cacule('mc',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
        ...
        case 'ETH':
            ...
            mcvar[bz][symbol].asks= temp.asks;
            mcvar[bz][symbol].bids= temp.bids;
            putask1to('mc',bz,'ETH',temp.bids[0][0]);
            var max=findmax(bz);
            symbol=symbol.replace('_','');
            var result=cacule('mc',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
