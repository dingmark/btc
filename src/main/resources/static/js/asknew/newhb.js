...
if ('WebSocket' in window) {
    websockethb = new WebSocket("ws://"+getContextPath()+"/test/hb");
}
else
{
 ...
 //   temp=getcnc(temp)
    ...
    switch (type) {
        case'BTC':
            ...
            putask1to('hb',bz,'BTC',temp.bids[0][0]);
            //找到这个币种下面卖1目前最大值
            var max=findmax(bz);
        ...
        case 'SDT':
            ...
            putask1to('hb',bz,'USDT',temp.bids[0][0]);
            //findmaxAsk('hb',bz,symbol,temp.asks[0]);
            //找到这个币种下面卖1目前最大值
            var max=findmax(bz);
            //该币种买1与卖1最大值比较，计算利差
        ...
        case 'ETH':
            ...
            hbvar[bz][symbol].asks= temp.asks;
            hbvar[bz][symbol].bids= temp.bids;
            putask1to('hb',bz,'ETH',temp.bids[0][0]);
           // findmaxAsk('hb',bz,symbol,temp.asks[0]);
            //找到这个币种下面卖1目前最大值
            var max=findmax(bz);
            //该币种买1与卖1最大值比较，计算利差
