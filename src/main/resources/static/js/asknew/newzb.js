...
if ('WebSocket' in window) {
    websocketzb = new WebSocket("ws://"+getContextPath+"/test/zb");
}
else
{
   ...
   // locateposition('bian',event.data);
    ...
    switch (type) {
        case'TC':
            ...
            putask1to('zb',bz,'BTC',temp.bids[0][0]);
            var max=findmax(bz);
        ...
        case 'DT':
            ...
            zbvar[bz][symbol].asks= temp.asks;
            zbvar[bz][symbol].bids= temp.bids;
            putask1to('zb',bz,'USDT',temp.bids[0][0]);
            var max=findmax(bz);
            symbol=symbol.replace('_','');
            var result=cacule('zb',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
        ...
        case 'QC':
            ...
            zbvar[bz][symbol].asks= temp.asks;
            zbvar[bz][symbol].bids= temp.bids;
            putask1to('zb',bz,'QC',temp.bids[0][0]);
            var max=findmax(bz);
            symbol=symbol.replace('_','');
            var result=cacule('zb',symbol,temp.bids[0][0],max);
            if(result.percent>=percent_base)
